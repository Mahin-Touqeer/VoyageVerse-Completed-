    ...
    if (auth.currentUser) {
      if (comment.value) {
        ...
        if (reviewImage.files.length) {
          ...
          // await updateDoc(doc(db, 'reviews',), {
          //   image: arrayUnion(...imageURLarr)
          // })
        }
...
function displayReview(x) {
  ...
  x.data().reviews.forEach(async reviewId => {
    ...
    console.log(review.photos[1]);

    if (review.photos.length) {
      // let reviewCard2 = document.createElement("span");
      // reviewCard2.classList.add('reviewCard'); 
      let abc = ``;
